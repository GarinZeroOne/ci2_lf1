...
class Stikis_model extends CI_model {
    ...
    function comprarStiki($idUser, $idPiloto, $tipoStiki) {

        //Si el idGp no es mayor que 0 la temporada está terminada
        if ($this->idGp > 0) {

            $sql = "SELECT * FROM stikis_usuarios WHERE id_usuario = ? AND id_gp = ?";

            $query_check = $this->db->query($sql, array($idUser, $this->idGp));

            // Comprobamos que no tenga ya 2 stikis para el GP
            if ($query_check->num_rows() < 2) {

                // Obtener fondos del usuario
                $fondos_usuario = $this->db->query("SELECT fondos FROM usuarios_banco WHERE id_usuario = ?", array($idUser))->row()->fondos;
                ...
                // Mejoras Mecanicos
                $valor_mejora = $this->mejoras_model->getValorMejora($idUser, 2);
                ...
                if ($fondos_despues_de_compra >= -200000) {
                    ...
                    $query_check2 = $this->db->query($sql_check2
                            , array($idUser, $this->idGp, $idPiloto));
                    ...
                    if ($query_check2->num_rows() == 0) {
                        ...
                        $this->db->query($sql, array(
                            $idUser, $this->idGp, $idPiloto, $tipoStiki, date('Y-m-d'),
                            'sin procesar')
                        );
                        ...
                        // devolver msg OK	
                        return "Stiki comprado correctamente!";
                    } else {
                        // el piloto ya tiene un STIKI para ese GP devolver msg
                        return "El piloto seleccionado ya tiene un Stiki para este GP";
                    }
                } else {

                    // Devolver msg NO TIENES PASTA SUFICIENTE
                    return "No tienes suficiente dinero para comprar el Stiki";
                }
            } else {
                // Devolver msg Ya tienes 2 stikis comprados
                return "Solo se pueden comprar 2 Stikis por GP";
            }
        }else{
            return "No hay mas GPs!";
        }
    }
